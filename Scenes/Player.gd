...
func switch_camera_type(type):
	...
	if type == 1:
		if rememberPerspective == [null,null,null]:
			...
			position.x = camera2dPos.x * mapToWorldScale
			position.z = camera2dPos.y * mapToWorldScale
			position.y = 5.0
		else:
...
func _input(event):
	...
	if event is InputEventMouseMotion and Input.get_mouse_mode() == Input.MOUSE_MODE_CAPTURED and oCamera3D.projection == Camera3D.PROJECTION_PERSPECTIVE:
	...
	if oCamera3D.projection == Camera3D.PROJECTION_PERSPECTIVE:
		speed_multiplier = 1
	else:
		speed_multiplier = 2
	...
	if Input.is_action_just_pressed("change_3d_mouse_mode") and oCamera3D.projection == Camera3D.PROJECTION_PERSPECTIVE:
...
func keyboard_pan():
	if oCamera3D.projection == Camera3D.PROJECTION_ORTHOGONAL: return
...
func _physics_process(delta):
	...
	velocity = velocity.lerp(direction * speed * speed_multiplier, acceleration * delta)
	movement = velocity
