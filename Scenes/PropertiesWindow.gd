...
func _on_PropertiesTabs_item_rect_changed():
	oPropertiesTabs.disconnect("item_rect_changed", Callable(self, "_on_PropertiesTabs_item_rect_changed"))
	var contentSize = vboxContainer.get_minimum_size()
	size = contentSize + Vector2(0,12)
	oPropertiesTabs.connect("item_rect_changed", Callable(self, "_on_PropertiesTabs_item_rect_changed"))
