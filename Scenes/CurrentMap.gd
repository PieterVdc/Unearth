...
func _init():
	get_window().set_title('Unearth v'+Version.full)
...
func set_path_and_title(newpath):
	if newpath != "":
		get_window().set_title(newpath + ' - Unearth v'+Version.full)
...
func clear_map(): # Remember, "Undo" calls this
	...
	print('Cleared map in '+str(Time.get_ticks_msec()-CODETIME_START)+'ms')
