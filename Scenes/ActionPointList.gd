...
func update_ap_list():
	...
	action_points.sort_custom(Callable(self, "sort_action_points"))
	...
	hero_gates.sort_custom(Callable(self, "sort_hero_gates"))
	...
	get_parent().get_parent().custom_minimum_size.y = 9 + (lines * item_height)
...
func _on_ActionPointListWindow_visibility_changed():
	...
	if oActionPointListWindow.visible == true:
		await get_tree().idle_frame
		oActionPointListWindow.position.x = 0
